...
def save_ex_cust_df(ex_cust_df, date):
    # Convert the date to a string in the format YYYY-MM-DD
    date_str = date.strftime('%Y-%m-%d')

    # Save the 'ex_cust_df' DataFrame as a CSV file with the generated filename
    ex_cust_df.to_csv(f'{file_path}ex_cust_df_{date_str}.csv', index=False)

    print(f"ex_cust_df saved for date {date_str} as ex_cust_df_{date_str}.csv")
